(async function () {
  ...
  function prose(s,a){html(s,(a||[]).map(x=>`<p>${esc(x)}</p>`).join(""))} function real(u){return u&&u!=="#"} function link(s,u){const n=document.querySelector(s);if(n){n.href=real(u)?u:"#";n.hidden=!real(u)}} function appNav(s,a,p="",q=""){const n=document.querySelector(s);n.href=`#${a.id}`;n.textContent=`${p}${a.name}${q}`} function artNav(s,a,p="",q=""){const n=document.querySelector(s);n.href=`#${a.id}`;n.textContent=`${p}${a.title}${q}`}
})();
